...
/**
 * Compatibility exports used by RUDAMasterPlan.jsx.
 * These keep the Master Plan Phases styling in one separate layer file.
 */
export const MASTER_PLAN_PHASES_COLOR = DEFAULT_RUDA_PLANNING_STYLE.color;

export const getMasterPlanPhasesOutlinePaint = (
  color = MASTER_PLAN_PHASES_COLOR,
  opacity = 1,
) => ({
  "line-color": color,
  "line-width": ["interpolate", ["linear"], ["zoom"], 6, 1, 10, 1.6, 14, 2.2],
...
export function addOrUpdateRudaPlanningBoundary(
  ...
  if (!map.getLayer(RUDA_PLANNING_BOUNDARY.fill)) {
    ...
    map.setLayoutProperty(RUDA_PLANNING_BOUNDARY.fill, "visibility", "visible");
  }
  ...
  if (!map.getLayer(RUDA_PLANNING_BOUNDARY.line)) {
    ...
    map.setPaintProperty(RUDA_PLANNING_BOUNDARY.line, "line-opacity", opacity);
    map.setLayoutProperty(RUDA_PLANNING_BOUNDARY.line, "visibility", "visible");
  }
