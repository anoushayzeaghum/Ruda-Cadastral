...
export default function FieldPoints() {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const res = await getFieldPoints();
      setItems(featureRows(res).map(asProperties));
    } catch (error) {
      console.error("Failed to load field points:", error);
      setItems([]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-black font-semibold text-[#10203a] dark:text-white md:text-2xl">
              Field Points Management
            </h1>
            <p className="mt-0.5 text-xs text-slate-500">
              Import field point shapefiles and review existing field point records.
            </p>
          </div>
        </div>
        ...
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative w-full sm:w-[360px]">
            ...
            <input
              ...
              placeholder="Search name, coordinates, mauza..."
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-400 dark:bg-white/5">
                  <tr>
                    ...
                    <th className="px-4 py-3">Easting</th>
                    <th className="px-4 py-3">Northing</th>
                ...
                <tbody>
                  {shown.map((row, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-500">{display(row.easting)}</td>
                      <td className="px-4 py-3 text-slate-500">{display(row.northing)}</td>
                      <td className="px-4 py-3 text-slate-500">{display(row.elevation)}</td>
