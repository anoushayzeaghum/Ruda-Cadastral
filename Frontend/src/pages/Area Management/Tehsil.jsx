...
import { getTehsils } from "../../services/api";
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-lg border p-6 bg-white dark:bg-[#07111a]">
        ...
        <p className="text-sm text-gray-500 mt-1">Create a new tehsil or edit an existing one.</p>
        ...
        <div className="mt-6 grid grid-cols-12 gap-4 items-center">
          ...
          <div className="col-span-5">
            <label className="block text-xs text-gray-500 mb-1">TEHSIL NAME</label>
            <input className="w-full rounded-md border px-3 py-2 text-sm bg-white dark:bg-[#0b1419]" placeholder="Enter tehsil name" />
          </div>

          <div className="col-span-3 flex gap-3 justify-end">
            <button className="bg-red-600 text-white px-4 py-2 rounded-md">+ Create Tehsil</button>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={3} className="py-6 text-center">No tehsils found</td>
                </tr>
              ) : (
                items.map((d, idx) => (
                  <tr key={d.id ?? d.gid} className="border-t">
                    <td className="py-3 w-12">{idx + 1}</td>
                    <td className="py-3 font-medium">{d.name ?? d.tehsil ?? "-"}</td>
