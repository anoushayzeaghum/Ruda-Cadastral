    ...
    <div className="pointer-events-none absolute right-3 top-4 z-30 flex flex-row-reverse items-start gap-2">
      ...
      <div className="pointer-events-auto flex flex-col gap-2">
        ...
        <PanelIcon
          ...
          onClick={() =>
            setActivePanel(activePanel === "vectorBoundaries" ? "" : "vectorBoundaries")
          }
          icon={<Building2 size={18} />}
        />
        <PanelIcon
          title="Raster Datasets"
          active={activePanel === "rasterDatasets"}
          onClick={() =>
            setActivePanel(activePanel === "rasterDatasets" ? "" : "rasterDatasets")
          }
...
function LayerManagerCard({ title, children }) {
  return (
    <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center gap-2 border-b border-slate-200 bg-slate-50 px-3 py-2.5">
        <h4 className="text-[12px] font-semibold leading-tight text-slate-700">
...
function SocietyLayerRow({
  ...
  return (
    ...
    >
      ...
      <div className="mt-0.5 flex items-center gap-2 pl-11">
        <input
          type="range"
          min="0"
          max="100"
      ...
      {disabled && disabledText && (
        <p className="mt-1 pl-11 text-[10px] text-slate-500">{disabledText}</p>
      )}
    </div>
  );
}
