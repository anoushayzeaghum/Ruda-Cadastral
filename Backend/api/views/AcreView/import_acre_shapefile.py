"""HTTP ZIP-upload wrapper for the Acre shapefile importer."""
...
@api_view(["POST"])
def import_acre_shapefile(request):
    try:
        with tempfile.TemporaryDirectory(prefix="ruda_acre_import_") as temp_dir:
            shp_path = _extract_shapefile(request.FILES.get("file"), temp_dir)
            imported = run_acre_import(shp_path)
        return _response("Acre imported successfully.", imported=imported)
    except ShapefileImportError as exc:
        return _response(str(exc), http_status=status.HTTP_400_BAD_REQUEST)
    except Exception as exc:
        return _response(
            f"Acre import failed: {exc}",
