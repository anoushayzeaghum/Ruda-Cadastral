"""HTTP ZIP-upload wrapper for the Square shapefile importer."""
...
from api.shapefiles.GISLayersShapefiles.import_square import ShapefileImportError, run_square_import
...
def _cleanup_work_dir(work_dir, retries=5, delay=0.20):
    ...
    The import result must not be reported as failed only because Windows,
    antivirus software, or GDAL briefly still has a shapefile sidecar open.
    """
    if not work_dir:
        return
    ...
    # Do not turn a successful DB transaction into HTTP 400 because only
    # temporary-file cleanup failed. Any remaining temp files can be removed
    # later by the OS / normal temp cleanup.
    shutil.rmtree(work_dir, ignore_errors=True)


@api_view(["POST"])
def import_square_shapefile(request):
    temp_dir = tempfile.mkdtemp(prefix="ruda_square_import_")

    try:
        shp_path = _extract_shapefile(request.FILES.get("file"), temp_dir)
        imported = run_square_import(shp_path)
        return _response("Square imported successfully.", imported=imported)
