"""HTTP ZIP-upload wrapper for the Field Points shapefile importer."""
...
def _cleanup_work_dir(work_dir, retries=5, delay=0.20):
    ...
    GDAL or antivirus software can briefly retain shapefile sidecar handles.
    Cleanup failure must not convert a successful DB import into HTTP 400.
    """
    if not work_dir:
        return
...
@api_view(["POST"])
def import_fieldpoints_shapefile(request):
    temp_dir = tempfile.mkdtemp(prefix="ruda_fieldpoints_import_")

    try:
        shp_path = _extract_shapefile(request.FILES.get("file"), temp_dir)
        imported = run_fieldpoints_import(shp_path)
        return _response("Field Points imported successfully.", imported=imported)
        ...
        return _response(
            f"Field Points import failed: {exc}",
